...
# put them together
trainDF <- trainActivity %>% transmute(Activity) %>% bind_cols(trainDF, subjectTrain)
...
# put them together
testDF <- testActivity %>% transmute(Activity) %>% bind_cols(testDF,subjectTest)
...
n <- names(fullDF)
tt <- grepl('mean[()]|std|Activity|Subject', names(fullDF))
n <- n[tt]
# clear the numbers
n <- gsub('[0-9]+ ', '', n)
# clear the ()
